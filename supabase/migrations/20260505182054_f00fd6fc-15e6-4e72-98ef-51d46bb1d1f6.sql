...
BEGIN
  ...
  FOR oi IN SELECT * FROM public.order_items WHERE order_id = NEW.id LOOP
    ...
    LOOP
      SELECT id, name, kind, master_row_count, normal_row_count, data
      INTO v_sheet
      FROM public.sheets WHERE id = sheet_link.sheet_id;

      IF v_sheet.id IS NULL THEN CONTINUE; END IF;

      -- Capacity check for family sheets (sheets with master rows / managers)
      IF COALESCE(v_sheet.master_row_count, 0) > 0 THEN
        SELECT COUNT(*) INTO v_current_count
        FROM jsonb_array_elements(COALESCE(v_sheet.data, '[]'::jsonb)) elem
        WHERE COALESCE(elem->>'kind', 'normal') = 'normal';

        SELECT COUNT(DISTINCT group_index) INTO v_group_count
        FROM public.sheet_master_rows WHERE sheet_id = v_sheet.id;

        v_capacity := COALESCE(v_group_count, 0) * COALESCE(v_sheet.normal_row_count, 0);
