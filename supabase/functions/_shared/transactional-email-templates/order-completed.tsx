...
// text-align coming from TipTap survives. Scripts/styles blocks and event
...
function sanitizeStyle(style: string): string {
  // Keep only a handful of safe declarations; drop anything with url(), expression(), or javascript:
...
function sanitizeAdminHtml(html: string): string {
  ...
  // Strip dangerous event handlers preemptively
  out = out.replace(/\son[a-z]+\s*=\s*"[^"]*"/gi, '')
  out = out.replace(/\son[a-z]+\s*=\s*'[^']*'/gi, '')

  // Rewrite opening tags: keep only allowed attributes per tag
...
// Encode a value into invisible zero-width characters so each email has a
// unique subject for Gmail (prevents threading/clipping) without showing
// the order ID visually.
...
const messageTitle = {
  ...
  margin: '0 0 8px', textAlign: 'center' as const,
}
const messageBox = {
