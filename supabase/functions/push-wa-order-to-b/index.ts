...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Expected: { order: {...}, items: [...], customer: {user_id, email, phone, name} }
    if (!body?.order || !body?.customer?.user_id || !Array.isArray(body?.items)) {
    ...
    if (!projectBUrl || !pushKey) {
      return new Response(JSON.stringify({ error: "PROJECT_B_SUPABASE_URL or PROJECT_B_PUSH_KEY not configured" }), {
    ...
    const resp = await fetch(url, {
      ...
      body: JSON.stringify({ type: "order", op: "insert", payload: body }),
    });
