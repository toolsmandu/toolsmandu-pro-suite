  ...
  const { data: assignments } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        ...
        .select('id, assigned_at, credential_id, order_id, validity_days, family_sharing_credentials(username, password, twofa_link, family_product_id, family_sharing_products(login_link, give_otp_access, products(name)))')
        .eq('user_id', user!.id);
      return (data || []) as any[];
    },
    enabled: !!user,
                          ...
                          {orderCreds.map((a: any) => {
                            ...
                            return (
                              ...
                              <div className="flex items-center justify-evenly gap-4 flex-nowrap min-w-max lg:min-w-0 lg:flex-wrap">
                                  ...
                                  {cred?.family_sharing_products?.give_otp_access && (cred?.twofa_link ? (
                                    ...
                                    <button onClick={() => navigate('/dashboard/inbox', { state: { email: cred?.username } })}
                                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-purple-500/20 text-purple-400 text-xs font-medium hover:bg-purple-500/30 transition-colors">
                                      <RotateCcwKey className="h-3 w-3" /> Get OTP Code
                                    </button>
                                  ))}
